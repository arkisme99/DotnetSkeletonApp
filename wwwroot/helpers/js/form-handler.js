  ...
  $btn.click(function (event) {
    ...
    if (!form.checkValidity()) {
      ...
      $form.find(":input").each(function () {
        if (!this.checkValidity()) {
          const $inputWrapper = $(this).closest(
            ".form-group, .mb-3, .form-floating"
          );
          ...
          if ($feedback.length) {
            ...
            if (this.validity.tooShort) {
              customMessage = `Minimal ${this.getAttribute(
                "minlength"
              )} karakter diperlukan.`;
            } else if (this.validity.tooLong) {
              customMessage = `Maksimal ${this.getAttribute(
                "maxlength"
              )} karakter diizinkan.`;
            }
    ...
    if (captchaSiteKey != null) {
      grecaptcha.ready(function () {
        grecaptcha
          .execute(captchaSiteKey, { action: "subscribe_newsletter" })
          .then(function (token) {
            $(form).append(
